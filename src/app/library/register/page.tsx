...
export default function RegisterLibraryItemPage() {
  ...
  const fetchLibraryItem = async (id: number) => {
    try {
      const response = await fetch(`http://mot.erns.co.kr/api/library/${id}`);
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) {
        // 수정(UPDATE)
        response = await fetch(`http://mot.erns.co.kr/api/library/${editId}`, {
          method: 'PUT',
          body: formData,
        });
      } else {
        // 신규 등록(CREATE)
        response = await fetch('http://mot.erns.co.kr/api/library', {
        method: 'POST',
        body: formData,
      });
      }
