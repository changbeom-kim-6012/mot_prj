...
export default function LearningPage() {
  ...
  const fetchLearningCategories = async () => {
    try {
      ...
      const basicResponse = await fetch('http://motclub.co.kr/api/codes');
      ...
      const response = await fetch('http://motclub.co.kr/api/codes/learning-details');
  ...
  const fetchSubjects = async (categoryId: number) => {
    try {
      ...
      const response = await fetch(`http://motclub.co.kr/api/subjects/category/${categoryId}`);
  ...
  const fetchRelatedMaterials = async () => {
    try {
      const response = await fetch('http://motclub.co.kr/api/library');
      if (response.ok) {
        const data = await response.json();
