...
export default function LearningPage() {
  ...
  const fetchLearningCategories = async () => {
    try {
      ...
      const basicResponse = await fetch('http://127.0.0.1:8082/api/codes');
      ...
      const response = await fetch('http://127.0.0.1:8082/api/codes/learning-details');
  ...
  const fetchSubjects = async (categoryId: number) => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8082/api/subjects/category/${categoryId}`);
  ...
  const fetchRelatedMaterials = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8082/api/library');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleViewFile = (fileName: string, filePath: string) => {
    ...
    const fileUrl = `http://127.0.0.1:8082/api/library/view/${encodedPath}`;
