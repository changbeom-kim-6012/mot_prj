...
export default function OpinionsPage() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      ...
      try {
        ...
        console.log('Request URL:', 'http://mot.erns.co.kr/api/opinions');
        console.log('Current origin:', window.location.origin);
        console.log('Auth state:', { isAuthenticated, user: user?.email });
        
        const res = await axios.get('http://mot.erns.co.kr/api/opinions', {
          headers: {
        ...
        const attachmentPromises = filteredArticles.map(async (article: Article) => {
          try {
            const attachmentRes = await axios.get(`http://mot.erns.co.kr/api/attachments`, {
              params: {
                refTable: 'opinions',
                refId: article.id
              }
  ...
  useEffect(() => {
    ...
    fetch('http://mot.erns.co.kr/api/codes/agora-details', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
  ...
  const handleFileView = (filePath: string, fileName: string) => {
    ...
    const fileUrl = `http://mot.erns.co.kr/api/attachments/view/${encodedFileName}`;
