...
export default function DialoguePage() {
  ...
  const handleDeleteMessage = async (messageId: number) => {
    ...
    try {
      const response = await fetch(`http://mot.erns.co.kr:8082/api/dialogue/messages/${messageId}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleStatusChange = async (newStatus: 'OPEN' | 'CLOSED') => {
    ...
    try {
      const response = await fetch(`http://mot.erns.co.kr:8082/api/dialogue/rooms/${selectedRoom.id}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handlePublicChange = async (isPublic: boolean) => {
    ...
    try {
      const response = await fetch(`http://mot.erns.co.kr:8082/api/dialogue/rooms/${selectedRoom.id}/public`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
  ...
  const handleDeleteRoom = async () => {
    ...
    try {
      const response = await fetch(`http://mot.erns.co.kr:8082/api/dialogue/rooms/${selectedRoom.id}`, {
        method: 'DELETE',
      });
