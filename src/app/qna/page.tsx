...
export default function QnaPage() {
  ...
  useEffect(() => {
    fetch('http://motclub.co.kr/api/codes/menu/Q&A/details')
      .then(res => res.json())
      .then(data => {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      const url = `http://motclub.co.kr/api/questions?page=0&size=1000`;
  ...
  const fetchQuestionsBySearch = async (keyword: string) => {
    try {
      ...
      const url = `http://motclub.co.kr/api/questions/category/${selectedCategoryId}?keyword=${keyword}&page=0&size=1000`;
  ...
  const fetchQuestionsByCategory = async (categoryId: number | null) => {
    try {
      ...
      if (categoryId && categoryId > 0) {
        // 특정 카테고리 ID로 검색
        url = `http://motclub.co.kr/api/questions/category/${categoryId}?page=0&size=1000`;
        console.log('카테고리 ID 필터 적용됨:', categoryId);
        console.log('생성된 URL:', url);
      } else {
        // 전체 목록 조회
        url = `http://motclub.co.kr/api/questions?page=0&size=1000`;
      ...
      if (response.ok) {
        ...
        if (response.status === 403) {
          ...
          const fallbackUrl = `http://motclub.co.kr/api/questions?page=0&size=1000`;
  ...
  const handleViewDetail = async (question: Question) => {
    ...
    if (isAdmin) {
      ...
      try {
        ...
        // 질문 상세 정보 불러오기
        const response = await fetch(`http://motclub.co.kr/api/questions/${question.id}`);
  ...
  const handleSubmitAnswer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://motclub.co.kr/api/questions/${selectedQuestion.id}/answers`, {
        method: 'POST',
        headers: {
