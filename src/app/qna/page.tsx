...
export default function QnaPage() {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      const url = `http://192.168.0.101:8082/api/questions?page=0&size=1000`;
  ...
  const fetchQuestionsBySearch = async (keyword: string) => {
    try {
      ...
      const url = `http://192.168.0.101:8082/api/questions/category/${selectedCategoryId}?keyword=${keyword}&page=0&size=1000`;
  ...
  const fetchQuestionsByCategory = async (categoryId: number | null) => {
    try {
      ...
      if (categoryId && categoryId > 0) {
        // 특정 카테고리 ID로 검색
        url = `http://192.168.0.101:8082/api/questions/category/${categoryId}?page=0&size=1000`;
        console.log('카테고리 ID 필터 적용됨:', categoryId);
        console.log('생성된 URL:', url);
      } else {
        // 전체 목록 조회
        url = `http://192.168.0.101:8082/api/questions?page=0&size=1000`;
      ...
      if (response.ok) {
        ...
        if (response.status === 403) {
          ...
          const fallbackUrl = `http://192.168.0.101:8082/api/questions?page=0&size=1000`;
