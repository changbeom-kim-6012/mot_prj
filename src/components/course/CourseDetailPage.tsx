...
export default function CourseDetailPage({ course }: CourseDetailPageProps) {
  ...
  const fetchMaterials = async (itemKey: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:8082/api/course-materials?courseId=${courseId}&sectionId=${sectionId}&topicId=${topicId}&itemId=${itemId}`);
      if (response.ok) {
  ...
  const handleUpload = async (itemKey: string) => {
    ...
    try {
      ...
      for (const fileWithType of files) {
        ...
        const response = await fetch('http://localhost:8082/api/course-materials', {
          method: 'POST',
          body: formData,
        });
  ...
  const handleDelete = async (itemKey: string, materialId: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:8082/api/course-materials/${materialId}`, {
        method: 'DELETE',
      });
  ...
  const handleDownload = async (material: CourseMaterial) => {
    try {
      const response = await fetch(`http://localhost:8082/api/course-materials/download/${material.id}`);
      if (response.ok) {
  ...
  const handleViewFile = (fileName: string, materialId: number) => {
    ...
    const fileUrl = `http://localhost:8082/api/course-materials/view/${encodedPath}`;
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {viewModalOpen && viewingFile && (
          <FileViewer
            ...
            fileUrl={`http://localhost:8082/api/course-materials/view/${encodeURIComponent(viewingFile.filePath).replace(/[!'()*]/g, function(c) {
              return '%' + c.charCodeAt(0).toString(16);
            })}`}
