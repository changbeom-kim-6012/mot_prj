...
export default function LibraryPage() {
  ...
  // 카테고리 불러오기
  useEffect(() => {
    fetch('http://localhost:8080/api/codes/menu/Library/details')
      .then(res => res.json())
      .then(data => {
  ...
  const handleDeleteItem = async (item: LibraryItem) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/library/${item.id}`, {
        method: 'DELETE',
      });
                              ...
                              {!isViewOnly && (
                                <button
                                  onClick={isAuthenticated && user ? () => {
                                    ...
                                    window.open(`http://localhost:8080/api/library/download/${encodedPath}`, '_blank');
                                  } : undefined}
                                  disabled={!isAuthenticated || !user}
                                  className={`inline-flex items-center px-2 py-1 border text-xs font-medium rounded ${
                                    isAuthenticated && user
