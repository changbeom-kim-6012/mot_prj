...
export default function LibraryPage() {
  ...
  // 카테고리 불러오기
  useEffect(() => {
    fetch('http://127.0.0.1:8082/api/codes/menu/Library/details')
      .then(res => res.json())
      .then(data => {
  ...
  const handleDeleteItem = async (item: LibraryItem) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8082/api/library/${item.id}`, {
        method: 'DELETE',
      });
                                 ...
                                 {!isViewOnly && (
                                   <button
                                     onClick={isAuthenticated && user ? () => {
                                       ...
                                       window.open(`http://127.0.0.1:8082/api/library/download/${encodedPath}`, '_blank');
                                     } : undefined}
                                     disabled={!isAuthenticated || !user}
                                     className={`inline-flex items-center px-2 py-1 border text-xs font-medium rounded ${
                                       isAuthenticated && user
