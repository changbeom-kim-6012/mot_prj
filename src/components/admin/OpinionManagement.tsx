...
export default function OpinionManagement() {
  ...
  const fetchOpinions = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://121.140.143.9:8082/api/opinions');
  ...
  const handleStatusUpdate = async (id: number, newStatus: string) => {
    try {
      ...
      let response;
      try {
        response = await axios.patch(`http://121.140.143.9:8082/api/opinions/${id}/status`, {
          status: newStatus
        }, {
        ...
        // 방법 2: 간단한 방식 (대안)
        response = await axios.put(`http://121.140.143.9:8082/api/opinions/${id}/status-simple?status=${newStatus}`);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await axios.delete(`http://121.140.143.9:8082/api/opinions/${id}`);
      fetchOpinions(); // 목록 새로고침
      alert('삭제되었습니다.');
    } catch (error) {
  ...
  const openAttachment = () => {
    if (selectedOpinion?.attachmentPath) {
      window.open(`http://121.140.143.9:8082/uploads/opinions/${selectedOpinion.attachmentPath}`, '_blank');
    } else {
      alert('첨부파일이 없습니다.');
    }
  };
