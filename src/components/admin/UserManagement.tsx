...
export default function UserManagement() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch('http://motclub.co.kr:8082/api/users');
        if (!response.ok) {
  ...
  const handleSaveClick = async (userId: number) => {
    ...
    try {
      const response = await fetch(`http://motclub.co.kr:8082/api/users/${userId}`, {
        method: 'PUT',
  ...
  const handleBulkSave = async () => {
    ...
    try {
      const response = await fetch('http://motclub.co.kr:8082/api/users/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleUpdateCreatedAt = async () => {
    ...
    try {
      const response = await fetch('http://motclub.co.kr:8082/api/users/update-created-at', {
        method: 'POST',
      });
      ...
      // 사용자 목록 새로고침
      const usersResponse = await fetch('http://motclub.co.kr:8082/api/users');
      if (usersResponse.ok) {
