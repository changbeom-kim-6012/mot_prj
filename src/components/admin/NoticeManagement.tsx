...
export default function NoticeManagement() {
  ...
  const fetchNotices = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8082/api/notices');
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:8082/api/notices/${id}`, {
        method: 'DELETE',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingNotice 
        ? `http://localhost:8082/api/notices/${editingNotice.id}`
        : 'http://localhost:8082/api/notices';
